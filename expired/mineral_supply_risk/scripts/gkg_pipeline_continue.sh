...
n = publish.run('inhouse/data_lake/db/minerals.duckdb', 'events')
print('발행행수', n)
"

echo "=== [4/7] 최종 SRS 재검증(n=200) ==="
python3 -m mineral_supply_risk.scripts.srs_post_cleanup_check --db inhouse/data_lake/db/minerals.duckdb --out-csv /tmp/srs_verify2_final.csv --seed 0.77 --n 200

echo "=== [5/7] geo_index/geo_prob 재계산 ==="
python3 -m geo index
python3 -m geo prob
python3 -m geo publish --db inhouse/data_lake/db/minerals.duckdb --what index

echo "=== [6/7] 수급진단 모델 재학습·평가 ==="
cd mineral_supply_risk
MSR_DB=../data_lake/db/minerals.duckdb python3 -m scripts.diagnosis_retrain_answer
cd ..
