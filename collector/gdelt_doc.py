# -*- coding: utf-8 -*-
"""[0] GDELT DOC API 뉴스 수집 → $COLLECT_OUT/inbox 텍스트 투척 (komis/collectors/gdelt.py 이식).
...
logger = logging.getLogger("collector.gdelt")
...
def run(minerals: list[str] | None = None, days: int = 90) -> int:
    ...
    while w <= limit:
        ...
        w += timedelta(weeks=1)
    save_seen(seen, "news")
    print(f"[gdelt] inbox 신규 투척 {total}건 → $COLLECT_OUT/inbox/gdelt/ "
