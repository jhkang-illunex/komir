...
산출: $COLLECT_OUT/bundles/collect_YYYYMMDD.zip   (2026-07-12 zip으로 확정 — 사용자 지정)
  내부 경로: inbox/<subdir>/<파일>.txt  +  gkg/<YYYY>/<ts>.gkg.csv.zip
...
def run(day: str | None = None) -> int:
    ...
    dest = BUNDLES / f"collect_{day}.zip"
    if dest.exists():        # 같은 날 재실행 → 증분 번들(suffix)
        n = 2
        while (BUNDLES / f"collect_{day}_{n}.zip").exists():
            n += 1
        dest = BUNDLES / f"collect_{day}_{n}.zip"

    part = dest.with_suffix(".part")
    with zipfile.ZipFile(part, "w", zipfile.ZIP_DEFLATED) as zf:
        for p, arc in files:
            zf.write(p, arcname=arc)
    # 재검증: 멤버 수 일치 + CRC 무결성 확인 후에만 원자적 rename + 원본 이동
    with zipfile.ZipFile(part) as zf:
        bad = zf.testzip()
        n_members = len([i for i in zf.infolist() if not i.is_dir()])
    if bad is not None or n_members != len(files):
        part.unlink()
        raise RuntimeError(f"번들 검증 실패: 멤버 {n_members}/{len(files)}, bad={bad}")
    part.rename(dest)
