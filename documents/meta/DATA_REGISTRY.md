...
- ⚠ **의도적으로 갱신하지 않은 문서**: `documents/산출물/2026-W29_0713-0719/피드백기반_수정플랜_260716.docx`
  ...
  사용자 확인 완료).
- **report_gen 분석요약 — DB 프롬프트 기반 LLM 보고서 전체 재작성·반복 검수(JSON, 2026-08-27)**:
  `documents/산출물/2026-W35_0824-0830/report_gen_LLM보고서_반복검수_260827/`
  — 발주처 KOMIS 덤프 384콤보를 실 vLLM(gemma-4-26b-a4b)+`ai_cfg.cfg_prompt` DB 프롬프트로
  전부 재작성한 회차별 결과(콤보별 최종 Markdown 보고서·LLM 원출력·지침 위반 목록·폴백
  사유 분포). `round0_pilot/`(하네스 보정), `roundN_full/`(회차 N 전체). 하네스 스냅샷·판독
...
- **광물자원가격 여러 광종+비교광종 확인(2026-09-13)**:
  `documents/산출물/2026-W37_0907-0913/report_gen_비교광종_확인_260913/`
  — 정적 덤프에 compareMnrl 실캡처가 없어 두 광종 단일계열 응답을 KOMIS
  실제 응답 모양으로 합성(재현 스크립트에 합성 규칙 명시), 4메뉴 13건을
...
- **분석요약 개선작업 v10.pptx(2026-09-13)**:
  `documents/산출물/2026-W37_0907-0913/요약분석_정리결과물/
  분석요약_개선_결과작업_v10.pptx`(20슬라이드, evidence는
  `report_gen_v10_슬라이드_260913_evidence/`) — v9(12슬라이드)에 가격
  4종 비교광종 슬라이드 4개+지도 3종 옵션 슬라이드 4개(국가필터·생산품
  유형필터·수출입국가옵션·매장량생산량교차비교) 추가. 전부 실 report_gen
  호출 결과 그대로(손입력 없음). ⚠git 미커밋.
...
- **map_mineral 세계총계 버그(지도 국가합계 → KOMIS 공식 `_TOTAL_`) 수정
  (2026-09-13)**: `documents/산출물/2026-W37_0907-0913/
  report_gen_생산량총계_TOTAL_수정_260913/verify_world_total.py` — 사용자
  제보(생산량 총계 23,000,000 vs 보고서 요약 20,013,000)로 발견한 실제
  계산 버그. `_world_total()`이 지도(chart) 국가별 합계만 써 "그 밖의
  국가"(`_ETC_`) 생산량이 빠져 있었다 — 정적 덤프 87쌍 전수 스윕으로
  기존 주석("`_TOTAL_`이 지도합계보다 4~11배 작다", 3곳)이 정반대로
  틀렸음을 확인·정정. 수정 후 87/87 일치(원본 JSON 독립 재계산 대조),
  395콤보 하네스·유닛테스트 회귀 없음, 재배포 후 실 HTTP 확인 완료.
  상세는 WORKLOG 2026-09-13(최신) 항목. v10.pptx 슬라이드18·19·20도
  수정된 총계로 갱신(⚠git 미커밋, 위 항목과 동일 사유).
- **v10.pptx map_global 2개 슬라이드 라벨링 오류 수정(2026-09-13)**:
  `documents/산출물/2026-W37_0907-0913/
  report_gen_map_global_방향라벨_버그_260913/README.md` — 사용자 제보로
  발견. report_gen API 버그 아님(코드 확인 완료), v10.pptx 생성 스크립트
  쪽 라벨링 오류 2건: 슬라이드16(리튬 baseline)은 캡션 "수입"인데 실제
  데이터가 KOMIS "수출"(O) 탭 패턴이었던 구버전(v9부터) 데이터 —
  komis.or.kr I탭(수입) 라이브 재조회로 교체. 슬라이드17(동, 수출입국가
  옵션)은 방향은 정상이나 캡션에 "2026"이라 적은 게 실제로는 "전체기간
  (2014~2026 누적)"이었던 라벨 오기 — 캡션만 정정. KOMIS I(수입)/O(수출)
  탭 코드 매핑을 페이지 HTML에서 직접 실측 확정(기존 문서에 없던 정보).
  상세는 WORKLOG 2026-09-13(최신) 항목.
- **국내/글로벌 수급지도+광물자원가격 Playwright 화면-API 대조 점검
  (2026-09-13)**: `documents/산출물/2026-W37_0907-0913/
  report_gen_map_korea_global_화면대조_260913/`,
  `documents/산출물/2026-W37_0907-0913/report_gen_가격메뉴_화면대조_260913/`
  — komis.or.kr 실 페이지를 Playwright로 직접 조작해 화면 렌더링을
  report_gen 출력과 대조(map_korea 5건·map_global 3건·price_* 5건).
  실버그 0건, 애매한 구조적 갭 1건(map_global "수출" 탭 파이프라인 미연결)
  발견만. 비교광종 기능 최초 라이브 검증 완료. 상세는 WORKLOG 2026-09-13
  (최신) 항목.
- **글로벌수급지도 수입/수출 UI 신설(2026-09-13)**: `inhouse/streamlit_demo/
  komis_fetch.py::fetch_map_global()`에 `trade_direction` 파라미터 추가
  (기존 "수입" 하드코딩 해소 — 이전 Playwright 점검에서 "애매한 갭"으로만
  보고했던 항목을 사용자 지시로 실제 구현), `views/report_demo.py`에
  수입/수출 라디오 UI 추가(report_gen 요청 바디엔 안 실음 — page_id=
  map_korea 전용 필드 제약). v10.pptx에 슬라이드17(신규, "수출 옵션")
  추가 — 슬라이드16(수입, 리튬 2026)과 나란히 비교되도록 배치(세계총액
  1.73억→12.41억 달러, 1위 루트 호주→인도네시아→호주→중국로 확실히
  다른 결과). 상세는 WORKLOG 2026-09-13(최신) 항목.
- **v10.pptx 광종 일관성 수정(2026-09-13)**: `documents/산출물/
  2026-W37_0907-0913/report_gen_v10_광종일관성_수정_260913/README.md` —
  사용자 지적으로 22개 슬라이드 전체를 "baseline과 옵션-변형이 같은
  광종을 쓰는가" 기준으로 감사, 4건 불일치(비철금속·희소금속 비교광종,
  국내수급지도 생산품유형필터, 글로벌수급지도 수출입국가옵션) 발견·전부
  baseline 광종으로 재구성(전부 실제 report_gen 호출). 표 숫자서식 버그
  2건도 부수 발견·수정. 상세는 WORKLOG 2026-09-13(최신) 항목.
- **v10.pptx 숫자서식 버그 재조치(2026-09-13)**: 광종 일관성 수정 중
  발견한 표기 버그 2건이 report_gen API가 아니라 슬라이드 빌드 스크립트의
  서식 함수 문제였음을 확인, 손패치를 폐기하고 서식을 고친
  `rebuild_v10_mineral_consistency_final.py` 하나로 슬라이드 3·5·15·18을
  처음부터 재생성(`report_gen_v10_광종일관성_수정_260913/` 폴더). 상세는
  WORKLOG 2026-09-13(최신) 항목.
- **v11.pptx 신설(2026-09-13)**: `documents/산출물/2026-W37_0907-0913/
  report_gen_v11_슬라이드_260913_evidence/README.md` — 수급동향지표
  기본/옵션추가(패널차트) 2슬라이드 분리 신설(23슬라이드), 국내수급지도
  국가필터 총액 산출 실버그(v10부터 있었음, sumIncmAmt 행단위 미반영)
  발견·수정, 소수점 트림 규칙 미반영 캡처 전체 재조회, 억/만 자동선택
  표기 개선. 스크립트 하나(`build_v11_full.py`)로 전체 재생성. 상세는
  WORKLOG 2026-09-13(최신) 항목.
- **v11.pptx 수출입국가 필터 슬라이드 추가(2026-09-13)**: PPT 20페이지
  ("수출입국가 옵션") 뒤에 국가필터(대한민국→인도네시아, 리튬 2026)
  신규 슬라이드 추가(24슬라이드). map_global의 export_country_name/
  import_country_name 필터를 슬라이드로 첫 시연. 상세는 WORKLOG
  2026-09-13(최신) 항목, `report_gen_v11_슬라이드_260913_evidence/
  README.md` 후속 추가 섹션.
- **v11.pptx 수급동향지표 "기본/옵션추가" 분리 되돌림(2026-09-13)**:
  사용자 지적으로 패널차트(komis_snapshot_response)가 진짜 KOMIS UI
  옵션이 아니라 기본 데이터임을 확인, 단일 슬라이드로 복귀(23슬라이드).
  상세는 WORKLOG 2026-09-13(최신) 항목.
- **v11.pptx 국내수급지도 "수입/수출 추이" 절 신설(2026-09-13)**:
  기존 `trade_scale_trend_fact` 기능(2026-09-10 신설)이 슬라이드 빌드
  스크립트에서 `komis_history_responses` 미전달로 계속 빠져 있던 것을
  사용자 지적으로 발견·수정. 세 슬라이드(baseline·국가필터·생산품유형
  필터) 전부 최근 5개년 추이 반영. 상세는 WORKLOG 2026-09-13(최신).
- **v12.pptx 신설(2026-09-13)**: `documents/산출물/2026-W37_0907-0913/
  report_gen_v12_슬라이드_260913_evidence/README.md` — "아규먼트는 다
  들어오는게 기본" 원칙으로 교차비교·수출입국가옵션 2슬라이드 삭제·
  baseline에 병합(21슬라이드). 상세는 WORKLOG 2026-09-13(최신) 항목.
- **report_gen 피드백 반영 감사 + D-10 수정(2026-09-13)**: `documents/
  산출물/2026-W37_0907-0913/report_gen_v12_슬라이드_260913_evidence/
  README.md`(감사 섹션) — D-10(기준일→조회기간 표기, 5일 미반영 발견·
  수정)+슬라이드 렌더링 불일치 3건 정정. 상세는 WORKLOG 2026-09-13
  (최신) 항목.
- **v12.pptx 광물자원가격 기준일 통일(2026-09-13)**: `documents/산출물/
  2026-W37_0907-0913/report_gen_v12_슬라이드_260913_evidence/README.md`
  (정정2 섹션) — price_* 4종이 각기 다른 시점 정적 덤프를 쓰던 걸 전부
  라이브 재조회로 교체(3종 2026-09-10 통일, 우라늄만 실제 데이터특성상
  8-31). 상세는 WORKLOG 2026-09-13(최신) 항목.
- **v12.pptx 전 메뉴 라이브 재조회 전수 대조(2026-09-13)**:
  `documents/산출물/2026-W37_0907-0913/report_gen_v12_슬라이드_260913_
  evidence/README.md`(정정3 섹션) — 광물종합지수만 실제 stale(8/24→
...
- **rag_chat 구조화 블록 공통 명세 + 추천 차트(2026-09-16)** ★현행 정본:
  ...
  line/bar/pie + 규칙), 실데이터 데모 블록 4종, 취소선 정책, `image` 이벤트
  제거. 상세는 WORKLOG 2026-09-16(최신).
- **rag_chat /prichat 구조화 블록 명세(2026-09-13)**: `documents/산출물/
  ...
  미적용). **2026-09-16 위 공통 명세로 대체됨(역사 기록)**. 상세는 WORKLOG
  2026-09-13.
- **report_gen 검수 → v13.pptx + 템플릿 문서 4종 재생성(2026-09-13)**:
  ...
  no_export_data D-10, 비교광종 표 행 노출)+슬라이드 스크립트 3건(표
  잔존값·행 부재·절 제목 접미어) 정정, 렌더러 직접 대조로 21슬라이드 절·
  본문·표 전부 일치. 템플릿 정본 4종(`report_gen_10페이지_업무지시서_준수
  점검_260909/01_메뉴별_최종_답변_템플릿.md`+`AI통계분석_요약답변_*.md`)은
  실제 출력 415건 대조로 재생성. 상세는 WORKLOG 2026-09-13(최신).
- **map_global "참고:" 접두어 제거(2026-09-13)**:
  API(`report_render.py`)+슬라이드+재배포+라이브 curl 검증 전부 완료.
  메뉴별 템플릿 정본은 신규 문서가 아니라 기존
  `documents/산출물/2026-W37_0907-0913/
  report_gen_10페이지_업무지시서_준수점검_260909/`의 2종
  (`01_메뉴별_최종_답변_템플릿.md`+`AI통계분석_요약답변_핵심광물지도.md`,
  둘 다 2026-09-09 신설)을 그대로 갱신 — 중복 신설했던 개요형 문서는
  사용자 지적으로 삭제. 상세는 WORKLOG 2026-09-13(최신) 항목.
