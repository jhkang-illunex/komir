...
**수정**: 소수점 1자리까지 반올림(`quantize(Decimal("0.1"))`)으로 바꿔
오차를 ±5% 수준으로 줄였다 — 정수로 딱 떨어지면(예: 정확히 2.0억)
".0"은 안 보이게 처리. "만" 단위도 같은 문제라 함께 수정(divisor만
다르고 로직 공유). `_validate_llm_summary`의 "약 N억/만" 매칭 정규식도
소수점 허용하도록 확장.

**검증**: pyflakes·unittest 14/14·395콤보 스모크 전부 통과. 경계값
(150,000,000~249,999,999·999,999,999·123,456,789,012) 직접 계산으로
"약 1.5억"~"약 10억"~"약 1,234.6억" 정상 확인, 실제 1.75억 교역총액
합성 데이터로 map_global 직접 렌더링해 "약 1.8억 달러" 확인(수정 전엔
"약 2억"이었을 값).
