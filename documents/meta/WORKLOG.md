...
**의도적으로 미룬 부분**(사용자 지시 "verify 우선, reformulate는 차후"):
verify 수정 후 재검증에서 "미래" 오판은 사라졌지만, "최근 6개월"처럼 상대
기간 표현이 실제 날짜범위로 안 바뀌어(ROUTE_PROMPT의 komis_start/end_period가
상대표현은 아직 null 처리, 2026-09-03 기존 설계) 기본 limit개(최근 수일치)만
조회되는 바람에 verify가 "특정 날짜 스냅샷일 뿐 넓은 추이가 아니다"로 간헐적
재기각하는 별개의 갭이 남아있다 — `chatbot_graph.py`의 REFORMULATE_PROMPT
앞 docstring에 후속 작업 메모 남겨둠(route/reformulate에도 오늘_날짜 배선 +
상대기간→절대기간 변환 필요).
