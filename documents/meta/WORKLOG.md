...
## 2026-09-26 — 챗봇 피드백 QA P0 출처·기간 게이트 (로컬 수정, 미배포)
...
회귀 검증은 변경 전후에 나눠 수행했다. 전체 관련 묶음은 108건 PASS였고, 마지막
응답 문구 조정 후 현재 소스로 실행한 action-contract audit, chart metadata,
가격응답 단일 케이스는 총 51건 PASS다. 해당 마지막 상태로 전체 묶음을 재실행했을
때 기존 LLM 의존 테스트 `test_current_diagnosis_q08_stops_before_retrieval`가
45초 제한 안에 종료되지 않아 전체 재검증은 미완료다. `py_compile` 및
`git diff --check`는 PASS. 앱·DB·색인은 변경하지 않았으며 운영 테스트 컨테이너에는
미배포다.
