...
> 정본 원칙: 이 문서는 `inhouse/data_lake/db/minerals.duckdb`(구 `warehouse/minerals.duckdb`,
> 2026-08-06 DMZ/inhouse 물리 분리로 이동 — 로컬 전용, git 미추적)를 **직접
> introspection**(`PRAGMA table_info` + `count(*)`)해서 작성했다.
> 행수·컬럼·타입은 **2026-07-22 기준 실측값**이며, DB가 갱신되면 이 문서도 갱신이 필요하다
> (재현: 아래 "갱신 방법" 참고). `inhouse/mineral_supply_risk/db/schema_core.sql`·
> `inhouse/mineral_supply_risk/data/raw/{00_schema,geo_schema}.sql`은 **구현 이전 설계 초안**(DDL
...
python3 -c "
import duckdb
con = duckdb.connect('inhouse/data_lake/db/minerals.duckdb', read_only=True)
...
`out_report` — 전부 `inhouse/mineral_supply_risk/db/schema_core.sql` 등 초기 설계 DDL에서 만들어진
...
적재: `scripts/load_market_aux.py`. 추가 src: 'AKSHARE_MACRO'(중국 PMI 2종)·'AKSHARE_MACRO2'(ISM·유로 PMI·중국 부동산)·'CFTC_SOCRATA'(COT 구리)·'BACKFILL_PUBLIC'(거시 6계열 2006~2021-06 과거분 — ECB·미 재무부·동방재부, 교차검증 후 KOMIS 이전 주만, 2026-07-24). ⚠ PK에 src가 없어 광종·날짜당 1소스만 가능 —
...
적재: `scripts/load_market_aux.py`. 추가 src: 'AKSHARE_MACRO'(중국 PMI 2종)·'AKSHARE_MACRO2'(ISM·유로 PMI·중국 부동산)·'CFTC_SOCRATA'(COT 구리)·'BACKFILL_PUBLIC'(거시 6계열 2006~2021-06 과거분 — ECB·미 재무부·동방재부, 교차검증 후 KOMIS 이전 주만, 2026-07-24).
