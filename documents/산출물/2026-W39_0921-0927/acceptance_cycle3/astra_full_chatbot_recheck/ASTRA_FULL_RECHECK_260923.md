...
- 소스 gate: 최초 집중 회귀 **85 PASS, subtest20 PASS** (`source_unit_gates.log`). 명시된 8개 파일 재실행은 **79 PASS, subtest20 PASS**이며 `source_reproducible_gates.log`에 별도 보존했다. 두 검사는 겹치므로 합산하지 않는다.
...
|---|---|---|---|
| AC01 | FAIL | FAIL | 가격 원값 일치. 더미65일을 최근1년 답으로 선택하며 순번·건수 표시가 남음 |
| AC02 | FAIL | FAIL | 가격 원값 일치. 실제 LME 기준도 존재하나 더미 선택·순번·건수 표시 |
| AC03 | FAIL | FAIL | 가격 원값 일치. 더미 선택·순번·건수 표시 |
| AC04 | FAIL | FAIL | 가격 원값 일치. 더미 선택·순번·건수 표시 |
| AC05 | FAIL | FAIL | 가격 원값 일치. 더미 선택·순번·건수 표시 |
| AC06 | FAIL | FAIL | 가격3근거 조회 후 unverified_or_incomplete_observation. 결합 답변 미제공; 독립적으로 완전 부재는 입증되지 않음 |
| AC07 | FAIL | FAIL | NI -3.27% 독립 계산 일치. chart y_unit=null, 메뉴 출처 누락; 가격 코드의 사용자 단위 해석 미완 |
| AC08 | FAIL | PASS | LI 수입상위5 금액·전체분모·비중 일치. 더미 고지 있음. 공통 표시 개선은 별도 |
| AC09 | FAIL | BLOCKED_DATA | 선택 광종 매핑에는 2025년7~9월 더미40행만 존재. 전체12개월 실제 월별 데이터 선행조건 미충족 |
| AC10 | FAIL | PASS | HS2603000000 품목명·수입액·중량·조회기간이 독립SQL과 일치. 전체광종 합계로 확대하지 않음 |
| AC11 | FAIL | BLOCKED_DATA | LI HHI는 dummy_trade_concentration으로 의도적 기권. 실제 관측 전체교역분모 확보 필요 |
| AC12 | FAIL | PASS | 2026 LI 상위5 매장량·전체분모 비중 일치. 더미 고지 있음 |
| AC13 | FAIL | PASS | 2026 REE 생산·매장 상위5와 비중 일치. 더미 고지 있음 |
| AC14 | FAIL | FAIL | 광산20문서 집계 응답 존재. 실제 OKF 사용에도 done의 4소스 모두 not_selected. 전체광산 순위의 독립 원본 완전대조는 미완 |
| AC15 | FAIL | PASS | PageIndex 인용과 OKF58행 Escondida in Chile 일치. source 이름 literal 비교는 검사기 오판 |
| AC16 | FAIL | FAIL | TSI -1.0 계산은 일치. 무차원 지표를 USD로 표시하고 더미·부분관측 고지 누락 |
| AC17 | FAIL | BLOCKED_DATA | 2024 LI 수입행 없음. 2025 증감률 분모가 없어서 기권은 적절 |
| AC18 | FAIL | FAIL | dependency 대신 unsupported_combination. 독립SQL상 중국28518400/전체108544000=26.2735849%; 더미 고지/실제자료 정책은 필요 |
| AC19 | PASS | PASS | RCA 세계분모 미연결에 대한 정상 기권. 계산 완료를 뜻하지 않음 |
| AC20 | PASS | PASS | TII 정상 기권 계약 충족. 로그 실제 중단점은 빈 필터값 거절; 세계분모 adapter 도달은 미검증 |
| AC21 | PASS | PASS | 무역TSI에 기준국·기간만 추가 질문 |
| AC22 | FAIL | FAIL | 같은 session 후속 요청에서 UnboundLocalError; done 없이 SSE 종료 |
| AC23 | PASS | PASS | 공개 가격 메뉴 안내·로그인 불필요 계약 충족 |
| AC24 | PASS | PASS | 공개 수입 메뉴 안내·로그인 불필요 계약 충족 |
| AC25 | FAIL | FAIL | 관련성 부족으로 기권. dense 타광종·PageIndex0·OKF unavailable. 코퍼스가 없다고 입증된 것은 아님 |
| AC26 | FAIL | FAIL | 삼원계 관계 근거 미회수로 기권. 최신 로컬 직접 검색에서도 무관한 큰 절이 상위에 존재 |
| AC31 | FAIL | FAIL | 사용자 문구는 정확한 정상 기권. SSE reason unsupported_mineral 대 YAML unsupported_commodity 및 message_key 누락 계약 불일치 |
| AC32 | PASS | PASS | 2030 실측가격0행 독립 확인. 미래실측을 만들지 않고 기권 |
| AC33 | FAIL | FAIL | 미가동 진단 기능에 정상 기권하나 expected_message_key action_unavailable가 SSE에 없음 |
| AC34 | PASS | PASS | 날씨·점심 무관 질문 정상 기권 |
| AC35 | PASS | PASS | 지원하지 않는 순위+HHI 조합 정상 기권 |
| AC36 | PASS | PASS | 공개 프로필에서 private 수급지표 제공하지 않음 |

## 재현 명령과 종료 범위

```bash
PYTHONPATH=inhouse:inhouse/rag_chat python3 -m pytest -q inhouse/rag_chat/tests/test_acceptance_suite_runner.py inhouse/rag_chat/tests/test_acceptance_module_boundaries.py inhouse/rag_chat/tests/test_acceptance_integrations.py inhouse/ingest/tests/test_build_okf_documents.py inhouse/ingest/tests/test_build_pgvector_okf.py inhouse/ingest/tests/test_paths_registry.py inhouse/ingest/tests/test_pdf_parser.py inhouse/ingest/tests/test_build_pageindex_trees.py
PYTHONPATH=inhouse:inhouse/rag_chat python3 -m pytest -q inhouse/rag_core/tests/test_internal_knowledge_route.py
