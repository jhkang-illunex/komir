# -*- coding: utf-8 -*-
"""v19.pptx(2026-09-15, 대상 6 광물지도-생산량 내용 보강)의 소스 — 절차는 v18과
동일하되 map_mineral 2건의 `komis_snapshot_response`(반대 measure 교차비교용)만 바꾼다:
v13~v18은 정적 덤프 `동|생산량|map`(2019~2025 다년 조회라 KOMIS가 기간 합산값을 준
응답 — `models.py` komis_snapshot_response docstring의 "단일 연도 전제" 위반, 그래서
호주 생산량이 575만톤/4.35%로 나왔음)을 그대로 썼다. 이제 같은 덤프의 chart 응답
(행마다 burudgQuty·prdctnQuty를 둘 다 가짐)에서 2025년 행만 골라 단일연도 스냅샷을
만든다 — 실제 KOMIS 단일연도 getListMapMnrlData 응답과 같은 구조. 생산량 슬라이드에도
같은 방식으로 매장량 스냅샷을 넣는다(v18까지는 안 넣어 교차비교가 없었음). 코드
변경: world_total_recent_trend·reserve_production_ratio_low/high·volatility_country
근거 신설, top3_period_change 순위표기, top3_concentration 해석 구절. 나머지 19건은
v18과 동일해야 한다.

(이하 v18 원문)
v18.pptx(2026-09-15, 대상 5 광물지도-매장량 내용 보강)의 소스 — 입력·절차는
v17과 완전히 동일(코드만 바뀜: map_mineral의 world_total_trend·top3_period_change·
top3_concentration·extreme_increase_1/2·extreme_decrease_1/2·top_country_vs_others
근거 신설, 구 extreme_change_countries 대체). 나머지 19건은 v17과 동일해야 한다.

(이하 v17 원문)
v17.pptx(2026-09-15, 대상 4 글로벌수급지도 "KOMIS 차트 기준" 삭제)의 소스 — 입력·절차는
v16과 완전히 동일(코드만 바뀜: map_global country_yearly_trend 문장 접두어).

(이하 v16 원문)
v16.pptx(2026-09-15, 대상 3 국내수급지도 수입 집중도)의 소스 — 입력·절차는
v15와 완전히 동일(코드만 바뀜: map_korea import_concentration 문장).
...
def run(key, page_id, **kwargs):
    req = AnalysisSummaryRequest(request_id=f"v19-{key}", page_id=page_id, **kwargs)
...
def snapshot_2025(key):
    """chart 응답의 2025년 행만 — 단일연도 getListMapMnrlData 응답과 같은 구조(v19 docstring)."""
    return {"data": [row for row in r08(key)["data"] if str(row.get("crtrYr")) == "2025"]}
...
run(
    ...
    komis_snapshot_response=snapshot_2025("동|생산량|chart"),
)
run(
    ...
    komis_snapshot_response=snapshot_2025("동|매장량|chart"),
)

json.dump(out, open("documents/산출물/2026-W38_0914-0920/report_gen_v19_슬라이드_260915_evidence/v19_sources.json", "w", encoding="utf-8"), ensure_ascii=False, indent=2)
