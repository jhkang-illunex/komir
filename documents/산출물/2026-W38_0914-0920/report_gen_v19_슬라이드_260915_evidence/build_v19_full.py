...
+ v17.pptx 대비 변경 구간 적색 표시. 절차 동일, 소스만 v19_sources.json, 대조 기준 v17.
...
def rebuild_map_mineral_slide(slide, data, caption, total_row_label="현재 세계 매장량", table_scale="eok"):
    ...
    ranking = " ".join(
        claims[cid]
        for cid in (
            "current_leaders", "third_country", "cross_measure_comparison", "top3_period_change", "top3_concentration",
            "reserve_production_ratio_low", "reserve_production_ratio_high",  # v19(대상 6)
        )
        if cid in claims
    )
    extreme = " ".join(
        claims[cid]
        for cid in (
            "extreme_increase_1", "extreme_increase_2", "extreme_decrease_1", "extreme_decrease_2",
            "volatility_country",  # v19(대상 6)
            "top_country_vs_others",
        )
        if cid in claims
    )
...
R = load("documents/산출물/2026-W38_0914-0920/report_gen_v19_슬라이드_260915_evidence/v19_sources.json")
...
p.save(path)
print("v19.pptx 저장 완료:", path, "| 슬라이드", len(p.slides), "| 적색 표시", len(changed), "곳 | 템플릿 잔존 적색→흑색", cleared, "run")
for item in changed:
    print("  ", item)
