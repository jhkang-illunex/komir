# -*- coding: utf-8 -*-
"""v10.pptx -> v18.pptx 전체 재생성(2026-09-15, 대상 5 광물지도-매장량 내용 보강)
+ v17.pptx 대비 변경 구간 적색 표시. 절차 동일, 소스만 v18_sources.json, 대조 기준 v17.
`rebuild_map_mineral_slide`만 바뀜 — 국가별 순위 절에 top3_period_change·top3_concentration,
주요 변화 절에 extreme_increase_1/2·extreme_decrease_1/2·top_country_vs_others(구
extreme_change_countries 대체), 세계 매장량/생산량 현황 절은 core_diagnosis 전체를 그대로
쓰므로 world_total_trend가 자동 포함된다.

(이하 v17 원문)
v10.pptx -> v17.pptx 전체 재생성(2026-09-15, 대상 4 글로벌수급지도 "KOMIS 차트 기준" 삭제)
+ v16.pptx 대비 변경 구간 적색 표시. 절차 동일, 소스만 v17_sources.json, 대조 기준 v16.

(이하 v16 원문)
v10.pptx -> v16.pptx 전체 재생성(2026-09-15, 대상 3 국내수급지도 수입 집중도) + v15.pptx
대비 변경 구간 적색 표시. 슬라이드 절차는 v15와 동일, 소스만 v16_sources.json, 대조 기준 v15.

(이하 v15 원문)
v10.pptx -> v15.pptx 전체 재생성(2026-09-15, 대상 2 광물종합지수) + v14.pptx 대비
변경 구간 적색 표시.

v15에서 바뀐 것: ①소스 v15_sources.json(종합지수만 전체기간 덤프+start_date 필터 입력, 구성 광종
(가중치) 문구, 조회기간 평균 지수 metric) ②`rebuild_composite_slide`가 표에
"전월 대비"·"전년 대비"·"조회기간 평균 지수" 행을 API가 낼 때 복제 삽입
③대조 기준이 v13.pptx → v14.pptx. 잔존 적색→흑색 통일 규칙은 v14와 동일.

(이하 v14 원문)
v10.pptx -> v14.pptx 전체 재생성(2026-09-15) + v13.pptx 대비 변경 구간 적색 표시.

v14에서 바뀐 것: 슬라이드 구성·채우기 절차는 v13과 동일하고 소스만
v14_sources.json(비교광종 상대가치 문장 표기 변경 반영)이다. 사용자 지시
"v13은 두고, v14를 만들어서 v13과 차이나는 부분만 적색 폰트로" — 전부 채운
뒤 실제 v13.pptx를 열어 같은 슬라이드·같은 도형·같은 문단(표는 같은 행
라벨·열)의 텍스트를 대조하고, 달라진 문단은 단어 단위 diff로 바뀐 토큰만
적색(FF0000) run으로 쪼갠다(표 셀·제목·캡션은 셀 전체). v13에 없는 표 행·
문단은 통째로 적색. 서식은 원래 run의 rPr을 복제해 유지한다.
`KOMIR_ROOT`(기본 ".")로 pptx 폴더(요약분석_정리결과물, git 미추적) 위치를
지정할 수 있다.
...
def rebuild_map_mineral_slide(slide, data, caption, total_row_label="현재 세계 매장량", table_scale="eok"):
    ...
    claims = claims_by_id(data)
    core = " ".join(s["text"] for s in data["summary"]["core_diagnosis"])
    # v18(2026-09-15 대상 5) — 국가별 순위 절에 top3_period_change·top3_concentration,
    # 주요 변화 절에 방향별 2개국(extreme_*)+1위 vs 기타 총정리(구 extreme_change_countries 대체).
    ranking = " ".join(
        claims[cid]
        for cid in ("current_leaders", "third_country", "cross_measure_comparison", "top3_period_change", "top3_concentration")
        if cid in claims
    )
    extreme = " ".join(
        claims[cid]
        for cid in ("extreme_increase_1", "extreme_increase_2", "extreme_decrease_1", "extreme_decrease_2", "top_country_vs_others")
        if cid in claims
    )
    ...
    fill_body(box, entries)
    # v18 — 본문이 v17 약 500자 → 약 1,400자(발주처 요구 4항목 반영)로 늘어 12pt·5.12in
    # 상자(spAutoFit, 아래로만 자람)로는 슬라이드 아래로 넘친다(추정 46줄≈9in > 5.4in 여유).
    # 이 두 슬라이드만 상자를 표(좌 7.85in) 직전까지 넓히고(6.6in) 9pt로 줄인다 — 추정 26줄≈3.9in.
    from pptx.util import Inches, Pt  # noqa: E402
    box.width = Inches(6.6)
...
def rebuild_composite_slide(slide, data, caption, caption_donor_slide=None):
    core = " ".join(s["text"] for s in data["summary"]["core_diagnosis"])
    major = " ".join(s["text"] for s in data["summary"]["major_changes"])
    cur = " ".join(s["text"] for s in data["summary"]["current_position"])
    ...
    mapping = {
        ...
        # v15 — 2026-09-15 발주처 피드백(전주·전월·전년 대비 + 평균 지수 표출).
        # 템플릿 표엔 "전주 대비"까지만 있어 API가 내는 행을 렌더러 순서
        # (key_metrics 순)대로 앞 행 복제로 끼워 넣는다(v13 ensure_table_row 패턴).
        "전월 대비": km.get("monthly_composite_change"),
        "전년 대비": km.get("yearly_composite_change"),
        "조회기간 평균 지수": km.get("period_average_composite_index"),
    }
    mapping = {k: v for k, v in mapping.items() if v is not None}
    prev = "전주 대비"
    for label in ("전월 대비", "전년 대비", "조회기간 평균 지수"):
        if label in mapping:
            ensure_table_row(table, label, prev)
            prev = label
    set_table_by_label(table, mapping)
    # 복제 삽입된 행은 앞 행("전년 대비")의 단위 셀("%")을 물려받는다 — 평균 지수는
    # 포인트 단위라 단위 셀을 API metric.unit으로 바로잡는다.
    if "조회기간 평균 지수" in mapping:
        for row in table.rows:
            if row.cells[0].text.strip() == "조회기간 평균 지수":
                for para in row.cells[2].text_frame.paragraphs:
                    if para.runs:
                        set_paragraph_text(para, mapping["조회기간 평균 지수"].get("unit") or "포인트")
                        break
    if caption:
        # v15 — 종합지수 슬라이드(v10 템플릿)엔 검색 옵션 캡션 도형이 없다. 입력 창이
        # 바뀐 사실을 독자가 봐야 하므로 시장동향지표 슬라이드(다음 장)의 캡션
        # "TextBox 11"을 복제해 같은 자리에 붙이고 문구만 바꾼다.
        try:
            cap = shape_by_name(slide, "TextBox 11")
        except KeyError:
            donor = shape_by_name(caption_donor_slide, "TextBox 11") if caption_donor_slide is not None else None
            if donor is None:
                return
            slide.shapes._spTree.append(copy.deepcopy(donor._element))
            cap = shape_by_name(slide, "TextBox 11")
        set_caption(cap, caption)
...
def _split_para_by_diff(para, old_text):
    """문단 텍스트를 v13 텍스트와 토큰(공백/비공백) 단위로 대조해, 바뀐 토큰만
    적색 run으로 분리한다. 첫 run의 rPr(글꼴·크기·굵기)을 복제해 서식을
    유지하고 나머지 run은 제거한다. 반환: 적색으로 칠한 텍스트 조각 목록."""
    new_text = _para_text(para)
    if not para.runs or new_text == old_text:
        return []
    base_r = para.runs[0]._r
    for extra in list(para.runs[1:]):
        extra._r.getparent().remove(extra._r)
    new_tokens = _TOKEN_RE.findall(new_text)
    old_tokens = _TOKEN_RE.findall(old_text)
    segments = []  # (text, is_changed)
    sm = difflib.SequenceMatcher(a=old_tokens, b=new_tokens, autojunk=False)
    for tag, _i1, _i2, j1, j2 in sm.get_opcodes():
        chunk = "".join(new_tokens[j1:j2])
        if not chunk:
            continue
        if segments and segments[-1][1] == (tag != "equal"):
            segments[-1] = (segments[-1][0] + chunk, segments[-1][1])
        else:
            segments.append((chunk, tag != "equal"))
    if not any(c for _, c in segments):  # 토큰화 차이만(이론상) — 문단 전체 적색
        segments = [(new_text, True)]
    parent = base_r.getparent()
    anchor = base_r
    reds = []
    for idx, (text, is_changed) in enumerate(segments):
        r_el = base_r if idx == 0 else copy.deepcopy(base_r)
        if idx > 0:
            anchor.addnext(r_el)
        anchor = r_el
        run = next(r for r in para.runs if r._r is r_el)
        run.text = text
        if is_changed:
            _color_run(run)
            reds.append(text.strip())
        elif idx > 0:
            # 복제된 run이 앞 세그먼트의 색을 물려받지 않도록 색 지정 해제
...
def _table_map(table):
    """{(행 라벨, 열 인덱스): 셀} — 같은 라벨이 여러 행이면 등장 순서로 번호를 붙인다."""
    out = {}
    seen = {}
    for row in table.rows:
        label = row.cells[0].text.strip()
...
def mark_diff_red(new_prs, old_prs):
    """new_prs(v14)의 텍스트를 old_prs(v13)와 슬라이드 순서·도형 이름 기준으로
    대조해 달라진 곳만 적색으로 칠한다. 반환: 사람이 읽을 변경 목록."""
    changed = []
    assert len(new_prs.slides) == len(old_prs.slides), (len(new_prs.slides), len(old_prs.slides))
    for si, (ns, os_) in enumerate(zip(new_prs.slides, old_prs.slides), start=1):
        old_shapes = {sh.name: sh for sh in os_.shapes}
        for shape in ns.shapes:
            old = old_shapes.get(shape.name)
            if shape.has_text_frame:
                new_paras = list(shape.text_frame.paragraphs)
                old_paras = list(old.text_frame.paragraphs) if old is not None and old.has_text_frame else []
                for pi, para in enumerate(new_paras):
                    old_text = _para_text(old_paras[pi]) if pi < len(old_paras) else ""
                    reds = _split_para_by_diff(para, old_text)
                    if reds:
                        changed.append(f"슬라이드{si} {shape.name} 문단{pi + 1}: {' | '.join(reds)}")
            if getattr(shape, "has_table", False) and shape.has_table:
                new_cells = _table_map(shape.table)
                old_cells = _table_map(old.table) if old is not None and getattr(old, "has_table", False) and old.has_table else {}
                for key, cell in new_cells.items():
                    old_cell = old_cells.get(key)
                    new_text = cell.text.strip()
                    old_text = old_cell.text.strip() if old_cell is not None else None
                    if new_text != old_text:
                        _mark_cell_red(cell)
                        changed.append(f"슬라이드{si} {shape.name} 표[{key[0]}#{key[1]} 열{key[2]}]: {old_text!r} → {new_text!r}")
    return changed
...
path_v13 = glob.glob(f"{PPTX_DIR}/*v17.pptx")[0]  # 대조 기준 = v17(손대지 않음; 변수명은 v13 스크립트 호환)
path_v12 = path_v10.replace("v10", "v18")
...
R = load("documents/산출물/2026-W38_0914-0920/report_gen_v18_슬라이드_260915_evidence/v18_sources.json")
...
# 10 광물종합지수
rebuild_composite_slide(
    p.slides[10], R["indicator_composite"],
    "검색 옵션 : 2025-07-01~2026-08-24(KOMIS 정적 덤프 전체기간을 기간 필터 — 전주·전월·전년 비교 시점 확보용)",
    caption_donor_slide=p.slides[11],
)
...
p.save(path)
print("v18.pptx 저장 완료:", path, "| 슬라이드", len(p.slides), "| 적색 표시", len(changed), "곳 | 템플릿 잔존 적색→흑색", cleared, "run")
for item in changed:
    print("  ", item)
