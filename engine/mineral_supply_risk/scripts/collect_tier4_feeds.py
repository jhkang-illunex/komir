...
실행: MSR_DB=<warehouse> ECOS_API_KEY=<키> python -m scripts.collect_tier4_feeds
...
def collect_akshare(con) -> None:
    ...
    try:
        e = ak.macro_china_society_electricity()
...
def collect_ecos_ship(con) -> None:
    ...
    for item1, sc in [("I11ACQ", "KSH_ELEC_M"), ("I11ACU", "KSH_AUTO_M")]:
        d = ecos_api.fetch_series("901Y032", "M", "200601", end,
                                  item1=item1, item2="3")   # 출하 원지수
        if d.empty:
