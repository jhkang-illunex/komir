...
2026-08-06: dmz/ 물리분리 이관 시 상대임포트 깊이를 `.config`로 수정(구 경로
`msr/collectors/customs_api.py`에서는 `..config`=msr.config였으나, 신 경로
`dmz/msr_collectors/customs_api.py`에서는 `.config`=dmz.msr_collectors.config).
"""
