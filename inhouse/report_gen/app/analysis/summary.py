...
_FORBIDDEN_SUMMARY_TERMS = (
    ...
    "원인으로 분석",
    "함에 따라",
    # 2026-09-10 사용자 지적 — indicator_market/supply의 가격 변동 문장
    # ("최근 한 달 동안 가격은 X% 올랐습니다")이 원래 같은 문단의 직전
    # 문장을 가리키던 "같은"으로 시작하다 9592cb890으로 고쳐졌는데,
    # 프롬프트 예시 문구에 옛 표현이 남아 있어 LLM이 그걸 베껴 쓰는
    # 사고가 재발했다(cdd8ccf29로 예시는 고쳤지만, LLM은 temperature=0
    # 이어도 매 호출 프롬프트 전체 맥락에 따라 이 표현을 다시 지어낼
    # 수 있어 "동작할 때마다 나왔다 안 나왔다"는 사용자 관찰과 일치한다).
    # 프롬프트 지시만으로는 못 막아서, 이 어떤 evidence의 fact에도 없는
    # "같은 최근" 조합 자체를 출력 검증에서 차단해 나오면 규칙 기반
    # 폴백으로 안전하게 보낸다.
    "같은 최근",
)
