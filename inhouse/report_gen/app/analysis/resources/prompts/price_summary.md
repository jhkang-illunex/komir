...
- 근거 원문의 어색한 활용도 교정한다. `최고가 대비 [수치]% 낮음으로`는
  `최고가보다 [수치]% 낮으며`로, `최근 가격 흐름은 단기와 중기 흐름이
  엇갈리는 상태입니다`는 `최근에는 단기와 중기 가격 흐름이 엇갈리고
  있습니다`로 다듬는다. 현재 가격 위치를 설명한 다음 최대 하락 구간은
  별도 문장으로 쓴다. `~이며 ... ~이며`를 거듭 이어 붙이지 않는다.
- 가격 요약의 각 평균 대비 비교에는 `높습니다`, `낮습니다`, `같은
  수준입니다`처럼 방향을 반드시 밝힌다. 방향이 다른 전주·전월·전년
  비교를 묶어 `[등락률]% 수준입니다`로 끝내지 않는다. 숫자와 부호는
  근거대로 유지하며, 서로 다른 방향은 별도 절이나 문장으로 나눈다.
  근거가 이미 `9.60% 낮습니다`처럼 방향을 명시하면 그 방향을 생략하거나
  `-9.60% 수준입니다`처럼 부호만 있는 표현으로 되돌리지 않는다.
- 2026-09-09 발주처 피드백(오전 2차, "가격요약 통합")으로 core_diagnosis가
  ...
  [상승/하락]했습니다. 전주평균([평균가]) 대비 [등락률]% [높습니다/
  낮습니다/같은 수준입니다], 전월평균([평균가]) 대비 [등락률]% [높습니다/
  낮습니다/같은 수준입니다], 전년평균([평균가]) 대비 [등락률]% [높습니다/
  낮습니다/같은 수준입니다]."처럼 하나의 문단으로 쓴다(2026-09-10 문장흐름
  수정 — evidence 자체가 이미 "높습니다"/"낮습니다"/"같은 수준입니다"로
  방향을 명시한 완성 문장이니 그대로 옮긴다. "…% 수준입니다"처럼 방향
  없이 숫자만 남기지 않는다). 각 비교의 상승·하락·보합 방향을 근거대로
  명시하고, 방향이 다른 비교는 나누어 대조한다. 이 예시는 어순 참고용이며
  없는 비교기간을 만들어 채우지 않는다. evidence의 등락률 숫자를 그대로
  옮기고 평균값·차이를 새로 계산해 적지 않는다. 이 페이지는 한 문장에 근거 5개까지
...
- compare_overall_change 근거가 있으면(비교광종 지정 시) current_position에
  두 번째 문장으로 "같은 조회기간 동안 [비교광종]은 [등락률]% 상승/하락한
  반면, [광종]은 [등락률]% 상승/하락했습니다"를 그대로 옮겨 쓴다(2026-09-11
  발주처 공통 개선요청[1]로 "변동한"·부호(+/-)가 "상승"/"하락"/"보합"
  방향 단어로 바뀌었다 — evidence 그대로 옮기면 자동으로 이 형태다) —
  어느 쪽이 더 크게 움직였는지는 숫자로만 드러내고, 원인이나 상관관계를
  추정하지 않는다.
- major_changes는 period_overall_change(조회기간 시작 대비 변화)를 먼저 설명한 뒤,
  price_streak와 ma_trend가 있으면 최근 구간 설명으로 이어 쓴다. 예를 들어
  `[시작일] 대비 [변화]했습니다. 최근에는 [N일 연속 하락]하고 있으며,
  [단기·중기 흐름]입니다`처럼 기간을 구분한다. 최근 구간의 두 근거는
  사실에 맞는 연결 어미로 묶어 같은 종결을 반복하지 않는다. 근거가 없는
  기간·지속·인과관계는 추가하지 않는다.
...
- ma_trend 근거가 있으면(2026-09-09 발주처 업무지시서 §3.1 "최근 변화" —
  평균 대비 위치) major_changes에 그대로 옮겨 쓴다 — 이미 "최근에는 단기와
  중기 가격 흐름이 모두 상승/하락 방향입니다" 또는 "최근에는 단기와 중기
  가격 흐름이 엇갈리고 있습니다"로 완성된 문장이니(2026-09-10 문장흐름
  수정으로 세 분기 모두 "최근에는 "로 시작하도록 통일 — price_streak와
  병합될 때 접두어가 겹치지 않게 하기 위해서다) 새 수치나 지표명(이동
...
- volatility·price_momentum·recovery_since_low·price_position·
  ...
  기본으로 하되, 위 "문장 연결" 지침에 따라 인접하면서 같은 주제를
  설명하는 근거만 자연스럽게 연결한다(사실·숫자·완성된 캐비엇 문구는 유지, 새 인과관계는
