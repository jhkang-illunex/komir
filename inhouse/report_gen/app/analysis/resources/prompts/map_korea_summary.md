...
- major_changes는 import_concentration 근거를 그대로 쓴다 — "수입
  ...
  새로 지어내 채우지 않는다.
  trade_scale_trend 근거가 있으면(2026-09-10 사용자 지시 — 신설, 과거
  연도 응답이 여러 건 있을 때만 등장) **항상 evidence_ids가 그 하나뿐인
  독립된 문장**으로 옮긴다 — 렌더링 단계가 이 evidence_id를 감지해 별도
  "## 수입·수출 규모 추이" 절로 분리하므로, import_concentration 등 다른
  근거와 한 문장에 섞으면 그 다른 근거의 내용까지 그 절로 잘못 끌려간다.
  이 근거는 "최근 N개년(YYYY~YYYY) 수입액 추이는 [연도] [금액] →
  [연도] [금액] → ...입니다. 같은 기간 수출액 추이는 [연도] [금액] →
  ...입니다."로 이미 완성돼 있다 — 수입·수출 두 문장을 절대 서로 바꿔
  쓰거나 섞지 않는다(사용자 지시 "수출/수입 잘 판별해서"). 연도·금액
  숫자를 하나도 빠짐없이 그대로 옮긴다.
