...
- supply_key_factors(있으면, current_position) — 국내 수입증가율(전년 대비
  실제 증감률)과 국내 수입국 편중도(상위 3개국 수입의존도 비중)다. 이 근거는
  2026-09-10부터 "화면상 확인되는 구성요소 중 [국내 수입증가율/국내 수입국
  편중도]의 변동이 상대적으로 크게 나타났습니다"(§3.2 스펙 문구, 계산 가능한
  두 요인 중 변동 크기가 더 큰 쪽을 계산기가 이미 골라 둔 것)로 시작한다 —
  이 선문장을 지우거나 순서를 바꾸지 않는다. 수입증가율 문장에서 퍼센트 수치는
  "국내 수입량"의 전년 대비 증감률이지 "수입증가율"
