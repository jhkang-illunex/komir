...
소스트리(`inhouse/report_gen/app/...` — shared는 4단 위의
...
def ensure_shared_on_path() -> Path:
    """`common/db.py`를 담은 디렉토리를 sys.path에 넣고 그 경로를 돌려준다."""

    here = Path(__file__).resolve()
    for candidate in (here, *here.parents):
        if (candidate / "common" / "db.py").is_file():
            if str(candidate) not in sys.path:
                sys.path.insert(0, str(candidate))
            return candidate
    raise ImportError(f"common/db.py를 {here} 상위에서 찾지 못함")
