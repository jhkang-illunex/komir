# -*- coding: utf-8 -*-
"""rel(발행처신뢰도) 실증 근거 보강(B-2) 재실행 — #3 (2026-07-24).

원본(`rel_source_tier_check.py`, 2026-07-16)과 완전히 동일한 방법론·코드를 그대로
재실행만 한다 — GKG 관련성 재정제(07-20~21, 71.4%→99.5%)와 시점정합성 수정(#8,
07-22)으로 geo_event가 크게 바뀐 뒤 결론이 유지되는지 확인 목적. 원본은 보존
(artifact-provenance 정책), 이 파일은 별도 산출물로 저장.
...
실행: python3 -m scripts.rel_source_tier_check_v2
산출: outputs/model_opt/rel_source_tier_check_v2.md
"""
...
def write_report(res: pd.DataFrame, n_ev: int, n_matched: int, n_unclassified: int):
    ...
    path = os.path.join(out_dir, "rel_source_tier_check_v2.md")
    L = []
    L.append("# rel(발행처신뢰도) 실증 근거 보강 — 정부공시 선행성 검증 (B-2)\n")
    L.append(f"작성: 2026-07-24(재실행, GKG 관련성 재정제+refdata 가동 후 최신 geo_event 기준) · B-1(`severity_sgn_empirical_check.md`)과 동일 방법론(주 단위 "
