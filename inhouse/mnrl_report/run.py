"""주간 실행 진입점(파이프라인). 문장은 engines/가 만들고, 이 모듈은 읽기→엔진→쓰기→기록만 한다.
    ...
    cd inhouse && python -m mnrl_report.run [--base-ymd 20260615] [--minerals MNRL0008,MNRL0002]
                                            [--scope all|overall|mnrl] [--llm] [--force] [--dry-run]
...
절차(보고 주차 base_ymd 1개):
 ...
 4. writer.upsert — DRAFT 행만 갱신, MANUAL은 비었을 때 고정 문안 시드,
    rule_ver/llm_model_ver에 엔진 버전(YYMMDD-sha8)을 찍는다
...
from .engines import GenEngine, RuleEngine
...
def collect_mineral_facts(code: str, name: str, base: date, cfg, ntn: dict) -> dict:
    return {
        ...
        "diag": sources.mineral_diag(code, base),
        "customs": sources.customs(code, base, cfg, ntn),
        "production": sources.usgs(code, base, "ko_rsrc_prdctn_quty", "prdctn_quty_ton", ntn),
        "reserve": sources.usgs(code, base, "ko_rsrc_burudg_quty", "burudg_quty_ton", ntn),
    }
...
def _produce(table: str, ctx: dict, rule_eng: RuleEngine, gen_eng: GenEngine | None, *,
             ...
             facts_path: str) -> dict:
    """행 1건: 규칙 → (생성형) → upsert → 실행 로그. 반환: 결과 요약 dict."""
    ...
    err = None
    try:
        status = writer.upsert(table, rule_res, rule_eng.version.ver, gen=gen_res,
                               gen_ver=gen_eng.version.ver if gen_eng else None,
...
def run(base: date, *, scope: str = "all", llm: bool | None = None, force: bool = False,
    ...
    facts_all: dict = {"base_ymd": base_ymd, "engines": engines_meta, "minerals": {}}
    results: dict = {"base_ymd": base_ymd, "engines": engines_meta, "mnrl": {}, "overall": None}
    common = dict(force=force, dry_run=dry_run, facts_path=facts_path, base_ymd=base_ymd)
    ...
    for m in minerals:
        ...
        if scope in ("all", "mnrl"):
            ...
            log.info("%s(%s): %s, RULE 채움 %d%s", name, code, out["status"], out["rule_cols_filled"],
                     f", GEN 채움 {out['gen_cols_filled']} 폐기 {len(out['gen_dropped'])}" if gen_eng else "")

    if scope in ("all", "overall"):
        facts_all["overall"] = sources.overall_diag(base)
        facts_all["gscpi"] = sources.macro("GSCPI", base)
        ctx = {"base": base, "code": None, "facts": facts_all, "news": []}
        seed = (fixed.get("ai_rpt_overall") or {}).get("_default")
