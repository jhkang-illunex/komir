...
- page: 사용자가 원하는 게 "화면·기능의 위치" 자체이거나, 화면의 데이터를
  ...
  3) "전체 데이터"·"원자료"처럼 화면의 데이터를 통째로 보여달라는 질문
     ...
     **"전체"·"원자료"·"통째로" 같은 표현과 함께 KOMIS가 실제로 다루는
     구체적 지표명이 둘 다 있어야만** 해당한다.
     예) "니켈 수급동향지표 전체 데이터 보여줘"
     반례) "니켈 데이터 보여주십시오"처럼 "전체"류 표현도 없고 어떤 지표
     (가격/수급동향/매장량 등)인지도 안 밝힌 막연한 요청은 page가 아니다 —
     이건 "질문이 애매해 조건을 특정할 수 없는" document 케이스다(아래
     document 목록의 "판단이 애매하면 document" 원칙 그대로 적용, 요구
     조건 재확인이 필요한 질문이지 화면 위치 안내로 해결될 질문이 아니다).
- document: 위 세 가지가 아니면 전부 document다. **개인정보·시스템 보안·
  투자자문처럼 KOMIS가 애초에 제공하지 않는 것을 요청하는 질문은 절대
  page가 아니다** — "다른 사용자의 조회 이력을 보여주십시오"·"관리자
  계정 정보를 알려주십시오" 같은 질문에 "보여주십시오"가 쓰였다고 화면
  안내 대상으로 착각하지 않는다(page는 KOMIS 공개 데이터 화면에 대해서만
  성립한다 — 애초에 존재하지 않는 화면을 "못 찾았습니다"로 안내하면
  보안 차단 자체가 무력화된다). 이런 질문은 반드시 document로 보내
  거절 사유가 정확히 분류되게 한다.
  ...
  - 어떤 지표인지 안 밝힌 막연한 요청, 그리고 개인정보·보안·투자자문
    요청도 document다(뒤 단계가 조건 재확인 또는 정확한 거절 사유로
    처리한다 — page로 보내면 "화면을 못 찾았습니다"라는 엉뚱한 응답이
    나온다).
    예) "니켈 데이터 보여주십시오", "다른 사용자의 조회 이력을
    보여주십시오", "관리자 계정 정보를 알려주십시오"
