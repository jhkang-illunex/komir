...
`common/llm_client.py`를 `common.llm_client`로 임포트하는데, 소스트리와 컨테이너
배포본의 상대 깊이가 달라 고정 depth로는 두 경우를 다 맞출 수 없다. 패키지 임포트
시점에 한 번 위로 훑어 찾는다(common/db.py·ingest/parsers/pdf.py가 쓰는 것과 같은
패턴)."""
...
def _find_shared_parent(start: Path) -> Path:
    """`common/llm_client.py`를 담은 디렉토리를 위로 훑어 찾는다."""
