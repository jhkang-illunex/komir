...
# (선택) [0] 자동 수집 → inbox 투척 — 2026-08-06 DMZ/in-house 분리로 dmz/geo_collectors로 이동됨
# (cd dmz && python -m geo_collectors collect-news --days 90 / collect-gdelt --days 90)
# 수집 결과(inbox 텍스트)가 망연계 게이트웨이를 통해 이 GEO_DATA/inbox로 전달된 이후부터
# 아래 [1]ingest부터 이어서 실행한다.
...
```bash
# (사전) 벌크 다운로드 — DMZ 몫(2026-08-06 dmz/geo_collectors로 이동). 별도 경로(대용량, NAS 등
# 권장). 5워커 연도단위 배정, 재개가능. (cd dmz && ...)
python -m geo_collectors.gkg_bulk_download --worker 0 --workers 5 --dest /path/to/bulk/gdelt &
...(worker 1~4 반복)
