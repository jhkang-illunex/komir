...
제외하고는 DB에서 값을 로딩하지 않는다"는 원칙에 따라, 8종 전부 `public.KO_*`
...
`INTERNAL_ERROR`) 하나로 성공/실패를 겸한다. **HTTP 상태 코드는 8종 전부 항상
...
class MineralDateRangeSummaryRequest(AnalysisEndpointRequest):
    ...
    `page_id="price"`(비철금속/희소금속) 전용: `price_criterion`(자유 텍스트,
    예: "LME CASH"·"Lithium Carbonate") — 두 그룹은 같은 광종이어도 조회조건
    (가격기준/품목·스펙)이 다를 수 있어, 어떤 조건으로 조회한 값인지 문자열로
    실어 보내면 보고서 상단에 그대로 표시된다(report_render.py).

    2026-08-26: `page_id="price"`(희소금속) 전용으로 `compare_*` 4종 필드도
    추가 — KOMIS 원본 API는 비교광종 지정 시 응답이 `data.defaultMnrl`(기본
    계열)·`data.compareMnrl`(비교 계열) 두 키로 온다(사용자 확인). 이 서버는
    `observations`=defaultMnrl 상당, `compare_observations`=compareMnrl
    상당으로 그대로 받는다 — 둘 다 있으면 보고서에 두 광종의 조회기간 변화율
    비교 문장이 추가된다(`komir_summary.py::calculate_price_summary`)."""
    ...
    # `page_id="price"`(희소금속) 전용 — KOMIS "비교광종" 기능 대응, 응답
    # docstring 참고.
...
@router.post("/prices", response_model=AnalysisReportResponse)
def summarize_price(
