...
#: 사용자 지시. `indicator_market`(KO_MRKT_PRSPECT_IDCT, 시장동향지표)·
#: `indicator_supply`(KO_SPDM_STBT_INDX, 수급동향지표) 2개 테이블만 private 전용이고,
#: 나머지 7개 `ko_*`(가격·가격예측·교역 2종·매장량·생산량·종합지수)는 public이다.
#: 매핑 메타데이터 테이블(`ai_hs_mnrl_map`·`ai_prc_mnrl_map`)은 특정 page_id로
#: 노출되지 않고 komis_raw.py 내부에서 코드→필터값 번역에만 쓰여 이 제한과 무관하다
#: (두 프로필 모두 공통으로 계속 쓴다).
PRIVATE_ONLY_KOMIS_PAGES: frozenset[str] = frozenset({"indicator_market", "indicator_supply"})
