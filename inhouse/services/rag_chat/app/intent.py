...
- document: 수집된 보고서·문서를 근거로 한 서술형 답(원인·배경·전망·정의·요약 등)을
  ...
  설명해줘", "DRC 광산 정책 변화 요약해줘", "코발트 광물종합지표의 최근 12개월
  변화를 보여줘"

예외(겉보기엔 page지만 document): "종합지표"·"위기지수"·"수급동향지표"·
"시장동향지표"처럼 **여러 지표를 합성·가공한 지수의 특정 시점 수치나 최근
변화**를 묻는 질문은 "보여줘"·"알려줘"로 끝나도 document다. 이런 합성지수는
화면의 원자료 표 하나로 안 끝나고 그 지수가 무엇을 근거로 어떻게
산출됐는지까지 설명해야 답이 되기 때문이다. 반면 "가격"·"수입량"·"수입금액"·
"생산량"·"매장량"처럼 그 자체로 하나의 원자료 수치·시계열인 지표는 이
예외에 해당하지 않고 위 page 기준을 그대로 따른다.

이 예외의 예외(다시 page): 대상이 합성지수여도 아래 두 경우는 여전히
page다 — (1) "어느 메뉴·화면에서 보나"처럼 화면 위치 자체를 묻는 질문
(예: "핵심광물지표는 어느 메뉴에서 확인할 수 있어?"), (2) "전체 데이터"·
"원자료"처럼 통째로 조회를 요청하는 질문(예: "니켈 수급동향지표 전체
데이터 보여줘") — 화면에서 위치를 알려주거나 원자료 전체를 그대로
보여주는 것 자체가 정답이라 산출 배경 설명이 필요 없기 때문이다.

핵심 구분: 정확한 수치·표·그래프 데이터(단, 합성지수의 특정 수치·변화는
제외) 조회를 원하면 page, 배경·원인·해석 서술이나 합성지수의 특정 수치·
변화 질문은 document다. "보여줘"·"알려줘"처럼 데이터 자체를 요청하는
동사는 page 쪽 신호이지만 대상이 합성지수면 위 예외가 우선하고(단, 위
"예외의 예외"에 해당하면 다시 page). "설명해줘"·"요약해줘"·"원인이 뭐야"
처럼 해석을 요청하는 동사는 document 쪽 신호다. 판단이 애매하면 document를
반환한다."""
