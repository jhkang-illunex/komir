...
def cached_ocr_pdf_text(pdf_path: str, data: bytes, cache_dir: str) -> str:
    """원본 해시별 OCR 결과를 재사용한다.

    이미지 중심 PDF는 Markdown 변환 결과가 있어도 OCR을 강제할 수 있다. 이때도
    기존 스캔본 폴백과 같은 캐시를 사용해 재색인 반복 시 OCR을 다시 수행하지 않는다.
    """
    import hashlib
...
def extract_with_fallback(pdf_path: str, data: bytes, md_text: str, cache_dir: str) -> tuple[str, str]:
    ...
    ocr_text = cached_ocr_pdf_text(pdf_path, data, cache_dir)
